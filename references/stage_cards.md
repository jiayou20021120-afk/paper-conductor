...
- **假设纪律：** 产出的假设要有方向、可证伪，且先于数据从理论推出（反 HARKing）；写不出方向性假设，多半还停在描述现象。见 [`references/hypothesis_discipline.md`](hypothesis_discipline.md)。
...
- **论证结构：** 按「理论预测 → 检验 → 证实/证伪」组织，不写成事后发现叙事；探索性工作可以重构，但预期必须真能独立于数据推导出来。见 [`references/hypothesis_discipline.md`](hypothesis_discipline.md)。
...
- **深修稿（guard 过零后）：** 语义层三查（自夸式元评论 / 句式 tic / 说明文腔）加论证过剩收敛（重复地图 / 隐喻限额 / 少写「我不是什么」）。工具双过不等于完成。见 [`references/deep_revision_pass.md`](deep_revision_pass.md)。
...
- **常见坑：** 把 AI 审稿当真同行评审。它是自查工具，不替代真实评审。另一个坑是返修越改越厚：逢审必改会堆出论证过剩，回写前先用 [`references/deep_revision_pass.md`](deep_revision_pass.md) 的判据扫一遍（这段在增加发现，还是在增加自我辩护），意见要选择性采纳。

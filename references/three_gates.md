...
| 3. 确定性 guard | 润色（5）/ 投稿（8） | `scripts/ai_tell_guard.py` | 破折号、直引号、本文密度、口水词，按固定规则零 token 复检，不让模型自评 |
...
- **WARN（提示，不拦截）：** 本文密度 > 约 1/千字、口水词（值得注意的是 / 综上所述 / 显而易见…）、枚举脚手架（首先，/ 其一，…）、单行全角括号堆叠，以及照 `paper-humanlike-checklist` 七症状补的：须字辈合规旁白（须注意 / 还须澄清…）、元叙述过渡腔（本文将 / 接下来将 / 如前所述…）、评论散文笔法（再往深处看 / 真正的问题或许是…）、自问自答（论证段问完立答）、设问开场（那么，/ 难道…）。研究问题用问号收尾不会误报。

**边界：** 它只查可程序化的硬指纹，不替代阶段 5 的语义润色，也不替代 `aiwei-zh` 的六维深洗。它是定稿前那道「机器先过一遍、人再过」的栅栏。改规则改 `scripts/ai_tell_guard.py` 顶部的 `EM_DASHES` / `STRAIGHT_QUOTES` / `FILLER` 列表即可。
